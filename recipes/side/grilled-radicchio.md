---
title: "Grilled Radicchio"
aliases: []
slug: grilled-radicchio
meal_type:
  - side
cuisine:
  - italian
course:
  - vegetable
dietary_tags:
  - vegan
  - vegetarian
  - gluten-free
occasion: []
season:
  - fall
  - winter
prep_time: "10 min"
cook_time: "10 min"
inactive_time: "0 min"
total_time: "20 min"
base_servings: 4
serving_unit: "portions"
scaling_notes: "The recipe scales linearly; use a grill large enough to accommodate the wedges in a single layer. For larger batches, grill in batches to avoid steaming."
source_type: "handwritten"
source_name: "Chef's Recipe Notebook"
source_url: ""
source_page: "24"
origin_notes: "Notebook side note from page 24. Radicchio wedges grilled until charred and caramelized, then dressed with balsamic and olive oil."
difficulty: "easy"
key_equipment:
  - grill
tags:
  - recipe/side
  - technique/grill
protein: []
status: reviewed
date_added: "2026-06-26"
date_modified: "2026-06-27"
---

# Grilled Radicchio

> [!NOTE] Origin
> Best-effort transcription from notebook page 24. Radicchio wedges grilled until charred and caramelized, then dressed with balsamic and olive oil — the high heat balances the bitterness.

---

## Ingredients

*Base servings: 4 portions*

- [ ] `2 heads` — radicchio, cut into quarters through the core (8 wedges total)
- [ ] `3 tablespoons` — olive oil, divided
- [ ] `1 teaspoon` — kosher salt
- [ ] `1 tablespoon` — balsamic vinegar
- [ ] `to taste` — freshly ground black pepper
- [ ] *Optional: 1/2 teaspoon minced garlic or 1 teaspoon honey for the dressing*

---

## Instructions

1. **Prepare the Radicchio.** Cut each head of radicchio in half through the core, then cut each half into quarters, keeping the core intact so each wedge holds together. You should have 8 wedges. Pat dry if wet.

2. **Make the Dressing.** In a small bowl, whisk together 1 tablespoon olive oil, 1 tablespoon balsamic vinegar, and a pinch of salt and pepper. Optional: add 1/2 teaspoon minced garlic or a teaspoon of honey for balance.

3. **Preheat the Grill.** Preheat a gas or charcoal grill to high heat (450–500°F / 230–260°C). The grates should be clean and lightly oiled to prevent sticking.

4. **Grill the Radicchio.** Brush the radicchio wedges on all sides with the remaining 2 tablespoons olive oil and sprinkle with salt. Place the wedges cut-side down on the hot grill. Grill undisturbed until the cut edges are deeply browned with grill marks and the leaves have wilted slightly but still hold structure (~3–4 min). Flip and grill the other side until warmed through and grill marks appear (~2–3 min). The radicchio should be charred at the edges but not burnt; it should smell smoky and sweet.

5. **Dress and Warm.** Transfer the grilled radicchio to a bowl. Pour the prepared dressing over the warm wedges and toss gently to coat. Let sit for 1 minute to allow the dressing to absorb slightly.

6. **Plate and Serve.** Arrange the dressed radicchio on a serving platter. Optionally, drizzle with any remaining dressing from the bowl and garnish with freshly cracked black pepper or shaved Parmesan if desired. Serve warm.

---

## Notes & Variations

### Cook's Notes
- Keeping the core intact ensures the wedges stay together on the grill and don't fall apart.
- The high heat caramelizes the natural sugars in radicchio, balancing its bitterness; watch closely to avoid burning, which can make it unpleasantly bitter.

### Variations
- **Anchovy-Garlic Dressing:** Mash 1 anchovy fillet with 1 clove garlic, then whisk with olive oil and balsamic vinegar for a umami-rich dressing.

### Make-Ahead
- Radicchio can be washed, cut, and stored dry in the refrigerator for up to 2 days; grill just before serving.
- Leftover grilled radicchio keeps refrigerated in an airtight container for up to 2 days; reheat gently in a skillet or serve cold in salads.

### Storage
- Refrigerate in an airtight container for up to 2 days. Reheat gently in a skillet or serve cold.

### Scaling
- The recipe scales linearly; use a grill large enough to accommodate the wedges in a single layer.
- For larger batches, grill in batches to avoid steaming.

---

## Linked Recipes

- None

*Source: Chef's Recipe Notebook, p.24 · Added: 2026-06-26*