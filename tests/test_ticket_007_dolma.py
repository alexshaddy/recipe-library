#!/usr/bin/env python3
import os
RECIPE_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "recipes")
RECIPE_PATH = os.path.join(RECIPE_DIR, "dinner/dolma.md")
"""
TICKET-007: Fix Dolma Ingredient Mismatch.

Validates `recipes/dinner/dolma.md` against:
  - TICKET-007 requirements
  - docs/specs/007-dolma-fix.md

Requirements from TICKET-007:
  1. File exists at recipes/dinner/dolma.md
  2. Notebook-specified ingredients: grape leaves, rice, ground lamb, onion,
     pine nuts, currants, dill, mint, lemon, olive oil, tomato paste.
     NO allspice, no cinnamon.
  3. Instructions: 5-8 steps with bolded titles, sensory cues, inline timing
  4. Cook's notes (>=2), variations (>=1), make-ahead/storage, scaling
  5. Frontmatter: cuisine middle-eastern, dietary_tags gluten-free,
     status reviewed, date_modified 2026-06-27
  6. Ingredients grouped (no sub-headings needed per spec, but all present)

Specific content checks:
  - Mentions "grape leaves", "rice", "ground lamb", "onion", "pine nuts",
    "currants", "dill", "mint", "lemon", "olive oil", "tomato paste"
  - Does NOT mention "allspice" or "cinnamon"
  - Status is "reviewed", date_modified is 2026-06-27
  - Tags include cuisine/middle-eastern
"""

import os
import re
import sys

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from test_helpers import *

# ── Constants ─────────────────────────────────────────────────────────────────

RECIPE = "dinner/dolma.md"

EXPECTED_TITLE = "Dolma"
EXPECTED_STATUS = "reviewed"
EXPECTED_DATE_MODIFIED = "2026-06-27"
EXPECTED_CUISINE = "middle-eastern"
EXPECTED_DIETARY_TAGS = ["gluten-free"]

NOTEBOOK_INGREDIENTS = [
    "grape leaves",
    "rice",
    "ground lamb",
    "onion",
    "pine nuts",
    "currants",
    "dill",
    "mint",
    "lemon",
    "olive oil",
    "tomato paste",
]

FORBIDDEN_INGREDIENTS = ["allspice", "cinnamon"]

REQUIRED_FRONTMATTER_FIELDS = [
    "title", "aliases", "slug", "meal_type", "cuisine", "course",
    "dietary_tags", "season",
    "prep_time", "cook_time", "inactive_time", "total_time",
    "base_servings", "serving_unit", "scaling_notes",
    "source_type", "source_name", "origin_notes",
    "difficulty", "key_equipment",
    "tags", "status",
    "date_added", "date_modified",
]


# ── Test Functions ────────────────────────────────────────────────────────────


def test_title(fm):
    """Title must be 'Dolma'."""
    assert fm.get("title") == EXPECTED_TITLE, (
        f"Expected title '{EXPECTED_TITLE}', got '{fm.get('title')}'"
    )


def test_status_reviewed(fm):
    """Status must be 'reviewed'."""
    assert fm.get("status") == EXPECTED_STATUS, (
        f"Expected status '{EXPECTED_STATUS}', got '{fm.get('status')}'"
    )


def test_date_modified(fm):
    """date_modified must be 2026-06-27."""
    dm = fmt_date(fm.get("date_modified"))
    assert dm == EXPECTED_DATE_MODIFIED, (
        f"Expected date_modified '{EXPECTED_DATE_MODIFIED}', got '{dm}'"
    )


def test_cuisine_middle_eastern(fm):
    """Cuisine must include 'middle-eastern'."""
    cuisines = fm.get("cuisine", [])
    if isinstance(cuisines, str):
        cuisines = [cuisines]
    assert any(EXPECTED_CUISINE in c.lower() for c in cuisines), (
        f"Expected cuisine to include '{EXPECTED_CUISINE}', got {cuisines}"
    )


def test_dietary_tags_gluten_free(fm):
    """dietary_tags must include 'gluten-free'."""
    tags = fm.get("dietary_tags", [])
    if isinstance(tags, str):
        tags = [tags]
    for exp in EXPECTED_DIETARY_TAGS:
        assert any(exp in t.lower() for t in tags), (
            f"Expected dietary_tags to include '{exp}', got {tags}"
        )


def test_tags_include_cuisine_middle_eastern(fm):
    """Tags must include cuisine/middle-eastern."""
    tags = fm.get("tags", [])
    if isinstance(tags, str):
        tags = [tags]
    assert any("cuisine/middle-eastern" in t.lower() for t in tags), (
        f"Expected a tag containing 'cuisine/middle-eastern', got {tags}"
    )


def test_tags_include_dinner(fm):
    """Tags must include recipe/dinner."""
    tags = fm.get("tags", [])
    if isinstance(tags, str):
        tags = [tags]
    tags_lower = [t.lower() for t in tags]
    assert any("recipe/dinner" in t for t in tags_lower), (
        f"Expected a tag containing 'recipe/dinner', got {tags}"
    )


def test_notebook_ingredients_present(content):
    """All notebook-specified ingredients must be present in the content."""
    lower = content.lower()
    missing = [ing for ing in NOTEBOOK_INGREDIENTS if ing not in lower]
    assert not missing, (
        f"Missing notebook ingredients in content: {missing}"
    )


def test_forbidden_ingredients_absent(content):
    """Forbidden ingredients (allspice, cinnamon) must NOT be present."""
    lower = content.lower()
    found = [ing for ing in FORBIDDEN_INGREDIENTS if ing in lower]
    assert not found, (
        f"Forbidden ingredients found in content: {found}"
    )


def test_instructions(content):
    """Instructions must have 5-8 steps with bolded titles, sensory cues, inline timing."""
    check_instructions(content, min_steps=5, max_steps=8)


def test_notes(content):
    """Must have >=2 cook's notes, >=1 variation, make-ahead/storage, scaling."""
    check_notes(content, min_cooks=2, min_variations=1)


def test_make_ahead(content):
    """Must have a Make-Ahead section."""
    assert "Make-Ahead" in content or "Make Ahead" in content, (
        "Missing 'Make-Ahead' section"
    )


def test_storage(content):
    """Must have a Storage section."""
    lower = content.lower()
    assert "storage" in lower, "Missing 'Storage' in content"


def test_scaling_section(content):
    """Must have a Scaling section."""
    assert "### Scaling" in content, "Missing '### Scaling' section"


def test_content_sections(content):
    """Check all required section headings exist."""
    required = [
        "## Ingredients",
        "## Instructions",
        "## Notes & Variations",
        "### Cook's Notes",
        "### Variations",
        "### Scaling",
    ]
    for heading in required:
        assert heading in content, f"Missing heading: '{heading}'"


def test_frontmatter_required_fields(fm):
    """All frontmatter fields must be present and non-empty."""
    for field in REQUIRED_FRONTMATTER_FIELDS:
        check_frontmatter_field(fm, field)


def test_source_documented(fm):
    """Check source_type and source_name are documented."""
    assert fm.get("source_type"), "source_type is missing or empty"
    assert fm.get("source_name"), "source_name is missing or empty"


def test_difficulty_set(fm):
    """Check difficulty is a valid value."""
    diff = fm.get("difficulty")
    assert diff in ["easy", "medium", "hard", "professional"], (
        f"Expected difficulty in [easy, medium, hard, professional], got '{diff}'"
    )


def test_key_equipment_present(fm):
    """Check key_equipment is populated."""
    ke = fm.get("key_equipment", [])
    assert len(ke) > 0, "key_equipment is empty or missing"


def test_origin_notes_present(fm):
    """Check origin_notes is populated."""
    on = fm.get("origin_notes", "")
    assert on, "origin_notes is missing or empty"


def test_slug_matches_filename(fm):
    """Check slug matches filename (without .md extension)."""
    expected_slug = "dolma"
    actual_slug = fm.get("slug", "")
    assert actual_slug == expected_slug, (
        f"Expected slug '{expected_slug}', got '{actual_slug}'"
    )


def test_base_servings_and_unit(fm):
    """Check base_servings and serving_unit are populated."""
    assert fm.get("base_servings") is not None, "base_servings is missing"
    assert fm.get("serving_unit", "") != "", "serving_unit is empty"


def test_inactive_time_set(fm):
    """Check inactive_time is set."""
    it = fm.get("inactive_time", "")
    assert it, "inactive_time is missing or empty"


def test_prep_cook_total_times_set(fm):
    """Check prep_time, cook_time, and total_time are set."""
    assert fm.get("prep_time"), "prep_time is missing or empty"
    assert fm.get("cook_time"), "cook_time is missing or empty"
    assert fm.get("total_time"), "total_time is missing or empty"


def test_origin_notes_references_notebook(content):
    """Origin notes or notebook reference must mention notebook ingredients."""
    lower = content.lower()
    assert "notebook" in lower, (
        "Content does not reference the notebook source"
    )


# ── Main ──────────────────────────────────────────────────────────────────────

if __name__ == "__main__":
    # Load recipe once; all subsequent tests use the same data
    fm, content = load_recipe(RECIPE)

    tests = [
        # Frontmatter completeness
        ("All required frontmatter fields", lambda: test_frontmatter_required_fields(fm)),
        ("Title", lambda: test_title(fm)),
        ("Status reviewed", lambda: test_status_reviewed(fm)),
        ("date_modified", lambda: test_date_modified(fm)),
        ("Slug matches filename", lambda: test_slug_matches_filename(fm)),

        # Cuisine & dietary
        ("Cuisine middle-eastern", lambda: test_cuisine_middle_eastern(fm)),
        ("Dietary tags: gluten-free", lambda: test_dietary_tags_gluten_free(fm)),

        # Tags
        ("Tags include cuisine/middle-eastern", lambda: test_tags_include_cuisine_middle_eastern(fm)),
        ("Tags include recipe/dinner", lambda: test_tags_include_dinner(fm)),

        # Source & metadata
        ("Source documented", lambda: test_source_documented(fm)),
        ("Difficulty set", lambda: test_difficulty_set(fm)),
        ("Key equipment populated", lambda: test_key_equipment_present(fm)),
        ("Origin notes present", lambda: test_origin_notes_present(fm)),
        ("Base servings and unit", lambda: test_base_servings_and_unit(fm)),
        ("Inactive time set", lambda: test_inactive_time_set(fm)),
        ("Prep/cook/total times set", lambda: test_prep_cook_total_times_set(fm)),

        # Notebook-ingredient checks
        ("Notebook ingredients present (grape leaves, rice, lamb, onion, pine nuts, currants, dill, mint, lemon, olive oil, tomato paste)",
         lambda: test_notebook_ingredients_present(content)),
        ("Forbidden ingredients absent (no allspice, no cinnamon)",
         lambda: test_forbidden_ingredients_absent(content)),

        # Instructions
        ("Instructions (5-8 steps, bolded titles, sensory cues, timing)",
         lambda: test_instructions(content)),

        # Notes & variations
        ("Cook's notes, variations, make-ahead, storage, scaling",
         lambda: test_notes(content)),
        ("Make-Ahead section", lambda: test_make_ahead(content)),
        ("Storage section", lambda: test_storage(content)),
        ("Scaling section heading", lambda: test_scaling_section(content)),

        # Content structure
        ("All required section headings", lambda: test_content_sections(content)),

        # Notebook reference
        ("Content references notebook source", lambda: test_origin_notes_references_notebook(content)),
    ]

    run_tests("TICKET-007", tests)
