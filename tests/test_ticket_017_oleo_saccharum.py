...
REQUIRED_CONTENT_MARKERS = [
    ...
    ("rest for oil extraction", r"\brest\b.*\b(?:1\s*to\s*2\s*hours?|oil\s*extraction)\b"),
...
def test_slug(fm):
    ...
    expected_slug = RECIPE_RELATIVE_PATH.replace("/", "-").replace(".md", "")
    expected_slug = expected_slug.split("-")[-1]  # "oleo-saccharum"
    # Actually just derive from filename
...
def test_notebook_ingredients(content):
    ...
    print("  ✓ All notebook-specified ingredient amounts found (50g peel, 200g sugar, 200mL water)")
...
def test_instructions_count(content):
    ...
    check_instructions(content, min_steps=MIN_INSTRUCTIONS, max_steps=MAX_INSTRUCTIONS)
    print(f"  ✓ Instructions count: within {MIN_INSTRUCTIONS}-{MAX_INSTRUCTIONS}")
...
def test_instructions_bolded_titles(content):
    ...
    print(f"  ✓ Bolded action titles ({len(steps)}): {[s.strip() for s in steps]}")
...
def test_instructions_sensory_cues(content):
    ...
    # Oleo-specific sensory cues
    sensory_terms = ["fragrant", "aroma", "aromatic", "glistening", "damp",
                     "cloudy", "syrupy", "perfume", "bright"]
...
def test_instructions_inline_timing(content):
    """Check instructions contain inline timing cues."""
    instr_match = re.search(r"## Instructions\s*\n(.*?)(?=## Notes|\Z)", content, re.DOTALL)
    assert instr_match, "Could not find Instructions section"
    instr_section = instr_match.group(1)
    # Look for timing patterns: numbers followed by min/hour
    timing = re.findall(r"\d+\s*(?:min|hour|hr|minute|second)s?", instr_section, re.IGNORECASE)
    timing += re.findall(r"\d+\s*to\s+\d+\s*(?:min|hour|hr|minute|second)s?", instr_section, re.IGNORECASE)
    assert len(timing) >= 3, (
        f"Expected ≥3 inline timing instances, found {len(timing)}: {timing}"
    )
    print(f"  ✓ Inline timing: {len(timing)} instance(s)")
...
def test_rest_period(content):
    ...
    content_lower = content.lower()
    # Look for "1 to 2 hours" or "1–2 hours" or "1-2 hours"
...
def test_cooks_notes(content):
    """Check for at least MIN_COOKS_NOTES cook's notes."""
    check_notes(content, min_cooks=MIN_COOKS_NOTES, min_variations=0)  # We'll check variations separately
...
def test_make_ahead_storage(content):
    ...
    assert "### Storage" in content or "# Storage" in content or "### Stor" in content, "Missing Storage section"
    print("  ✓ Make-Ahead / Storage sections present")


def test_scaling(content):
    """Check Scaling section with substantive content."""
    assert "### Scaling" in content or "## Scaling" in content, "Missing Scaling section"
    ...
    if scaling_match:
        ...
        # Could be at end of file or a different pattern
...
def test_content_structure(content):
    ...
    assert has_make_ahead or has_storage or has_combined, (
        "Missing '### Make-Ahead' or '### Storage' or combined heading"
    )
    print(f"  ✓ All required sections present ({len(required)}+ headings)")
...
def run_all_tests():
    ...
    # First check file exists before loading
    test_file_exists()

    # Load the recipe
    fm, content = load_recipe(RECIPE_RELATIVE_PATH)

    # Define all test cases as (name, callable) pairs
    tests = [
        ("File exists", test_file_exists),
        ("Frontmatter valid", lambda: (fm, content) or True),
        ...
        ("Notebook ingredients", lambda: test_notebook_ingredients(content)),
        ("Ingredient checkbox format", lambda: test_ingredients_checkbox_format(content)),
        ("Instructions count (5-8)", lambda: test_instructions_count(content)),
        ("Instructions bolded titles", lambda: test_instructions_bolded_titles(content)),
